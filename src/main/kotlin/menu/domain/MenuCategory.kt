...
enum class MenuCategory(
    ...
    JAPAN("일식", listOf("규동", "우동", "미소시루", "스시", "가츠동", "오니기리", "하이라이스", "라멘", "오코노미야끼")), KOREA(
        "한식",
        listOf("김밥", "김치찌개", "쌈밥", "된장찌개", "비빔밥", "칼국수", "불고기", "떡볶이", "제육볶음")
    ),
    CHINA("중식", listOf("깐풍기", "볶음면", "동파육", "짜장면", "짬뽕", "마파두부", "탕수육", "토마토 달걀볶음", "고추잡채")), ASIAN(
        "아시안",
    ...
    fun getRandomFood(inedibleFoods: List<String>): String {
        do {
            val randomFood = Randoms.shuffle(menus)[0]
            if (randomFood !in inedibleFoods) {
                return randomFood
            }
        } while (true)
    }

    fun isContain(menu: String): Boolean = menus.contains(menu)
