...
class MenuRecommender {
    ...
    private fun validateCategory(categories: List<Category>, category: Category): Boolean {
        if (categories.count { it == category } >= 2) return true
        return false
    }

    private fun generateMenuByCoach(coach: Coach, menus: List<Menu>, categories: List<Category>, dayOfWeek: Int): Menu {
        if (categories.size != 5 || dayOfWeek < 0 || dayOfWeek > 4) throw IllegalStateException(Message.ERR_MENU_ARGUMENT)
        while (true) {
    ...
    private fun validateMenu(coach: Coach, menu: Menu, menus: List<Menu>): Boolean {
        if (coach.validateMenu(menu)) return true
        if (menus.contains(menu)) return true
        return false
    }

    fun getRecommendation(categories: List<Category>, coaches: List<Coach>): Map<Coach, List<Menu>> {
    ...
    private fun getCoachRecommendation(
        menusByCoaches: MutableMap<Coach, MutableList<Menu>>, categories: List<Category>, dayOfWeek: Int, coach: Coach
    ) {
        if (menusByCoaches[coach] == null) menusByCoaches[coach] = mutableListOf()
